        ...
        for (const toolCall of response.tool_calls) {
            ...
            if (foundTool) {
                ...
                try {
                    ...
                    contentStr = `工具 ${toolCall.name} 调用失败: ${error.message || String(error)}`;
                }
...
await runAgentWithTools("成都世纪城附近的酒店，最近的 3 个酒店，拿到酒店图片，打开浏览器，展示每个酒店的图片，每个 tab 一个 url 展示，并且在把那个页面标题改为酒店名");
